...
@Entity     // JPA에 의해 관리되ㅑ는 엔티티임을 나타냄
@Table(name="items")    // 매핑된 데이터베이스 테이블( gallery 스키마의 items 테이블)의 지정
public class Item {
    @Id     // 아이디 필드, 해당 필드가 기본키 컬럼과 매핑됨을 나타냄
    ...
    @Column(updatable = false, nullable = false)    // 최초값 입력 후 수정되지 못함
    @CreationTimestamp       // 데이터 삽입시 값이 없다면 현재 시각 입력
